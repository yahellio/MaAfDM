...
function startAlgorithm() {
    ...
    if (isNaN(p_amount)|| p_amount < 1000 || p_amount > 100000) {
        alert("Пожалуйста, введите корректные значения для N.");
        return;
    }
...
function start(p_amount){
    all_points = []; 
    clasters = [];
    init(p_amount);
    maximinClustering(p_amount)
    drawClusters(ctx1);
    KMeans(k, p_amount);
    drawClusters(ctx2);
}

function maximinClustering(p_amount) {
    let first_center = all_points[randomInteger(0, p_amount - 1)];
    clasters.push({
        center: first_center,
        points: [],
    ...
    for (let i = 0; i < p_amount; i++) {
        ...
        let d = distance(clasters[0].center, point);
        if (d > max) {
            max = d;
            max_point = point;
        }
    ...
    do {
        ...
        let big_p = null;
        for (let j = 0; j < k; j++) {
            for (let point of clasters[j].points) {
                let dist = distance(clasters[j].center, point);
                if (dist > biggest) {
                    biggest = dist;
                    big_p = point;
                }
        ...
        for (let a = 0; a < k; a++) {
            for (let b = a + 1; b < k; b++) {
                sum_dist += distance(clasters[a].center, clasters[b].center);
            }
        }
        avg = sum_dist / (k * (k - 1) / 2);
